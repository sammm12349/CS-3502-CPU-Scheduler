...
4. Compare algorithm behaviors and outcomes";
        ...
        private void DisplaySchedulingResults(List<SchedulingResult> results, string algorithmName)
        {
            ...
            double cpuUtilization = totalTime > 0 ? (totalBurstTime / (double)totalTime) * 100 : 0;
            double throughput = totalTime > 0 ? (double)results.Count / totalTime : 0;

            var metricsItem = new ListViewItem("--- METRICS ---");
            metricsItem.SubItems.Add($"CPU Util: {cpuUtilization:F1}%");
            metricsItem.SubItems.Add($"Throughput: {throughput:F3} proc/unit");
            metricsItem.SubItems.Add($"Avg Wait: {avgWaiting:F2}");
            metricsItem.SubItems.Add($"Avg TAT: {avgTurnaround:F2}");
            ...
            /// </summary>    // Create a "Export Results" button in the results panel to save:
        ...
        private void ExportResultsToCSV()
        {
            ...
            {
                MessageBox.Show("No results to export. Run an algorithm first.",
            ...
            using (SaveFileDialog sfd = new SaveFileDialog())
            {
                ...
                sfd.FileName = "scheduling_results.csv";
                if (sfd.ShowDialog() == DialogResult.OK)
                {
                    var sb = new System.Text.StringBuilder();
                    sb.AppendLine("Process ID,Arrival,Burst,Start,Finish,Waiting,Turnaround");
                    foreach (ListViewItem item in listView1.Items)
                    {
                        var cols = item.SubItems.Cast<ListViewItem.ListViewSubItem>()
                                       .Select(s => s.Text);
                        sb.AppendLine(string.Join(",", cols));
                    }
                    System.IO.File.WriteAllText(sfd.FileName, sb.ToString());
                    MessageBox.Show("Results exported successfully!", "Export",
        ...
        private List<SchedulingResult> RunSRTFAlgorithm(List<ProcessData> processes)
        {
            ...
            while (completed < n)
            {
                ...
                if (!startTimes.ContainsKey(available.ProcessID))
                    startTimes[available.ProcessID] = currentTime;
                ...
                if (remainingBurst[available.ProcessID] == 0)
                {
                    ...
                    var turnaround = finish - available.ArrivalTime;
                    var waiting = turnaround - available.BurstTime;
                    results.Add(new SchedulingResult
                    {
                        ProcessID = available.ProcessID,
                        ArrivalTime = available.ArrivalTime,
                        BurstTime = available.BurstTime,
                        StartTime = startTimes[available.ProcessID],
                        FinishTime = finish,
                        WaitingTime = waiting,
        ...
        private List<SchedulingResult> RunHRRNAlgorithm(List<ProcessData> processes)
        {
            ...
            while (remainingProcesses.Count > 0)
            {
                ...
                var finishTime = startTime + nextProcess.BurstTime;
                var waitingTime2 = startTime - nextProcess.ArrivalTime;
                var turnaroundTime = finishTime - nextProcess.ArrivalTime;
        ...
        /// <summary>
        /// Executes the SRTF algorithm using DataGrid data.
        /// </summary>
